import Tracker.Serial.Modbus.functionsModbus as functions
import Tracker.Serial.Modbus.constsModbus as Const
...
class ModbusSerial:
    def __init__(self, uart_id, baudrate=9600, data_bits=8, stop_bits=1, parity=None, pins=None, ctrl_pin=None):
        ...
        if pinsLen==4:
            rts = pins[2]
            cts = pins[3]
            self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, stop=stop_bits, timeout_char=10, tx=tx, rx=rx, rts=rts, cts=cts)
        else:
            self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, stop=stop_bits, timeout_char=10, tx=tx, rx=rx)
        #self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, stop=stop_bits, timeout_chars=10, pins=pins)
    ...
    def write_single_coil(self, slave_addr, output_address, output_value):
        ...
        operation_status = functions.validate_resp_data(resp_data, Const.WRITE_SINGLE_COIL, output_address, value=output_value, signed=False)
    ...
    def write_multiple_coils(self, slave_addr, starting_address, output_values):
        ...
        resp_data = self._send_receive(modbus_pdu, slave_addr, False)
        operation_status = functions.validate_resp_data(resp_data, Const.WRITE_MULTIPLE_COILS, starting_address, quantity=len(output_values))
        return operation_status
    ...
    def write_multiple_registers(self, slave_addr, starting_address, register_values, signed=True):
        ...
        resp_data = self._send_receive(modbus_pdu, slave_addr, False)
        operation_status = functions.validate_resp_data(resp_data, Const.WRITE_MULTIPLE_REGISTERS, starting_address, quantity=len(register_values))
        return operation_status
