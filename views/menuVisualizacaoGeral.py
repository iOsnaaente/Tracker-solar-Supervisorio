...
def draw_sun_trajetory( draw_id, parent_id, all_day = False, extremes = False ):
    ...
    draw_line(     parent = draw_id, tag = id_link+1 , p1     = [center[0] - r, center[1]]              , p2     = [center[0] + r, center[1]]                                          , color = COLOR['gray'](155)  , thickness = 1 )
    draw_line(     parent = draw_id, tag = id_link+2 , p1     = center                                  , p2     = [center[0] + r*math.cos(azi-math.pi/2), center[1] + r*math.sin(azi-math.pi/2)], color = COLOR['orange'](155), thickness = 2 )
    draw_line(     parent = draw_id, tag = id_link+3 , p1     = center                                  , p2     = [center[0] + r*math.cos(alt-math.pi/2), center[1] + r*math.sin(alt-math.pi/2)], color = COLOR['gray'](200)  , thickness = 2 )
    draw_circle(   parent = draw_id, tag = id_link+4 , center = center                                  , radius = r                                                                   , color = COLOR['white'](200) , fill      = COLOR['white'](10 ), thickness = 3 )
    draw_circle(   parent = draw_id, tag = id_link+5 , center = center                                  , radius = 3                                                                   , color = COLOR['white'](200) , fill      = COLOR['white'](255), thickness = 2 )
    draw_text(     parent = draw_id, tag = id_link+6 , pos    = [center[0] -(r +20), center[1] -10 ]    , text   = 'W'                                                                 , color = COLOR['white'](200) , size      = 20 )
    draw_text(     parent = draw_id, tag = id_link+7 , pos    = [center[0] +(r +5) , center[1] -10 ]    , text   = 'E'                                                                 , color = COLOR['white'](200) , size      = 20 )
    draw_text(     parent = draw_id, tag = id_link+8 , pos    = [center[0] -10     , center[1] -(r +25)], text   = 'N'                                                                 , color = COLOR['white'](255) , size      = 20 )
    draw_polyline( parent = draw_id, tag = id_link+9 , points = dots                                    , color  = COLOR['red'](155)                                                    , thickness = 2                   , closed    = False )
    for n, p in enumerate(dots):
        draw_circle( parent = draw_id, tag = id_link+(12+n) , center = p     , radius = 2  , color = [n*4, 255-n*2, n*2, 255]                              ) 
    draw_line(       parent = draw_id, tag = id_link+10     , p1     = center, p2     = sun, color = COLOR['yellow'](200)    , thickness = 2               )
    draw_circle(     parent = draw_id, tag = id_link+11     , center = sun   , radius = 10 , color = COLOR['yellow'](155)    , fill = COLOR['yellow'](255) )

def update_sun_trajetory( draw_id, parent_id, all_day = False ): 
    # Ponto central, dimensões da tela e Raio 
    width, height = get_item_width( draw_id ), get_item_height( draw_id )
    w, h          = get_item_width( 'mainWindow' ) , get_item_height('mainWindow' )
    center        = [ width//2, height//2 ]
    r             = width//2 - 20 if width+20 <= height else height//2 - 20
    id_link       = draw_id*100
    ...
    configure_item( id_link+2 , p1     = center                    , p2     = [center[0] + r*math.cos(azi-math.pi/2), center[1] + r*math.sin(azi-math.pi/2)] )
    configure_item( id_link+3 , p1     = center                    , p2     = [center[0] + r*math.cos(alt-math.pi/2), center[1] + r*math.sin(alt-math.pi/2)] )
...
def init_visualizacaoGeral( windows : dict ):
    ...
    with window( label = 'Posição solar' , tag = 2_1_0, pos      = [50,50], width    = 500  , height      = 500 , no_move  = True, no_resize = True, no_collapse = True, no_close = True, no_title_bar= True ) as Posicao_sol_VG:
        ...
        add_drawlist      ( tag      = 2_1_1_0, width     = w-20  , height = h-50, label = 'Solar')
    ...
    with window( label = 'Atuação'       , tag = 2_2_0, no_move  = True   , no_resize = True, no_collapse = True, no_close = True ) as Atuacao_VG:
        ...
        with group( horizontal = True ): 
            with plot( tag = 2_2_1_0, label = 'Azimute do dia', height = 312, width = 478, anti_aliased = True ): 
                add_plot_legend()
                add_plot_axis( mvXAxis, label = 'Hora [h]'  , tag = 2_2_1_1, parent = 2_2_1_0 ) # X
                add_plot_axis( mvYAxis, label = 'Angulo [º]', tag = 2_2_1_2, parent = 2_2_1_0 ) # Y 
                set_axis_limits_auto( 2_2_1_1 )
    ...
    with window( label = 'Painel de log' , tag = 2_3_0, no_move  = True   , no_resize = True, no_collapse = True, no_close = True, no_title_bar = True ) as Painel_log_VG:
        ...
        with child_window( autosize_x = True, height = 165, menubar = True):
            ...
            with child_window( tag = 2_3_1_0):
                #Informações gerais do sistema - Automático 
                add_text('Hora automática')
                add_drag_floatx( label = 'Ano/Mes/Dia Auto'  , tag = 2_3_1, size = 3, format = '%.0f', speed = 0.1 , min_value = 1   , max_value = 3000   , no_input = True )
                add_drag_floatx( label = 'Hora/Min/Sec Auto' , tag = 2_3_2, size = 3, format = '%.0f', speed = 0.1 , no_input  = True                                                                             )
                add_drag_int   ( label = 'Valor no dia'      , tag = 2_3_3, format = '%.0f'          , speed = 0.1 , min_value = 0   , max_value = 26*3600, no_input = True, source = TOT_SECONDS, enabled = False)
                add_drag_int   ( label = 'Dia Juliano'       , tag = 2_3_4, format = '%.0f'          , speed = 0.1 , min_value = 0   , max_value = 366    , no_input = True, source = JULIANSDAY , enabled = False)
           
            with child_window( tag = 2_3_2_0):
                # Informações gerais do sistema - Manual  
                add_text('Hora manual')
                add_input_floatx( label = 'Ano/Mes/Dia Manual' , tag = 2_3_6, size = 3, default_value = [2020, 12, 25], format='%.0f', min_value = 1, max_value = 3000 )
                add_input_floatx( label = 'Hora/Min/Sec Manual', tag = 2_3_7, size = 3, default_value = [20, 30, 10]  , format='%.0f', min_value = 1, max_value = 60   )
                add_drag_int    ( label = 'Valor no dia'       , tag = 2_3_8, format = '%.0f', speed = 0.1 , min_value = 0, max_value = 24*3600, no_input = True, source = TOT_SECONDS, enabled = False )
                add_drag_int    ( label = 'Dia Juliano'        , tag = 2_3_9, format = '%.0f', speed = 0.1 , min_value = 0, max_value = 366    , no_input = True, source = JULIANSDAY , enabled = False )
        ...
        with child_window( tag = 2_3_3_0, autosize_x = True, autosize_y = True ): 
            ...
            with child_window( height = 90 ):
                ...
                add_input_float( label = 'Latitude' , tag = 2_3_10, min_value = -90, max_value = 90, format = '%3.8f', indent=0.01, source = LATITUDE , callback = lambda sender, data, user : SUN_DATA.set_latitude( data ) )
                dpg.add_spacer( )
            ...
            with child_window( height = 200 ):
                ...
                add_drag_floatx( tag = 2_3_16, size = 3, format='%.0f', speed=1, no_input= True, callback = lambda sender, data, user : dpg.set_value( H_SUNRISE     , data.extend([0]))  )
                dpg.add_spacer( )
                add_text('Culminante (hh/mm/ss)'   )
                add_drag_floatx( tag = 2_3_17, size = 3, format='%.0f', speed=1, no_input= True, callback = lambda sender, data, user : dpg.set_value( H_SUNSET      , data.extend([0]))  )
                dpg.add_spacer( )
...
def resize_visualizacaoGeral( ):
    ...
    configure_item( 2_1_0    , width = w*2/3   , height    = h*3/5       , pos = [10 , 25 ]               ) # DRAWING 
    configure_item( 2_2_0    , width = w*2/3   , height    = (h*2/5)-35  , pos = [10 , (h*3/5)+30 ]       ) # SUNPATH
    configure_item( 2_3_0    , width = w/3 -20 , height    =  h - 30     , pos = [ w*2/3 +15, 25 ]        ) # LOG 
    
    # get the child_window_window dimension 
    w1, h1 = get_item_width( 2_1_0 ), get_item_height( 2_1_0 ) 
...
def render_visualizacaoGeral( ):
    ...
    if dpg.get_value( HORA_MANUAL ) == False :
        SUN_DATA.update_date()
        dpg.set_value( 2_3_1, value = [ dpg.get_value(YEAR), dpg.get_value(MONTH), dpg.get_value(DAY)    ] )  # DIA ATUTOMÁTICO 
        dpg.set_value( 2_3_2, value = [ dpg.get_value(HOUR), dpg.get_value(MINUTE), dpg.get_value(SECOND)] )  # HORA AUTOMÁTICA
        hide_item( 2_3_2_0 )
        show_item( 2_3_1_0 )

    # Horário manual 
    else:        
        yearm, monthm, daym     = dpg.get_value( 2_3_6 )[:-1]
        hourm, minutem, secondm = dpg.get_value( 2_3_7 )[:-1]
        ...
        try: 
            ...
            print( 'Data inválida - visualização geral SUN_DATA.set_date()') 

        # Total de segundos no dia
        dpg.set_value( 2_3_9, SUN_DATA.dia_juliano   )                              # DIA JULIANO
        dpg.set_value( 2_3_8, SUN_DATA.total_seconds)                               # TOTAL DE SEGUNDOS 
        
        hide_item( 2_3_1_0 )
        show_item( 2_3_2_0 )
        
    # Setar o Azimute, Altitude e Elevação
    dpg.set_value( 2_3_12, math.degrees( SUN_DATA.azi) )                            #  AZIMUTE               
    dpg.set_value( 2_3_13, math.degrees( SUN_DATA.alt) )                            #  ALTITUDE               
    dpg.set_value( 2_3_14, SUN_DATA.altitude           )                            #  ELEVAÇÃO
    ...
    dpg.set_value( 2_3_16, [ SUN_DATA.rising.hour+SUN_DATA.utc_local , SUN_DATA.rising.minute , SUN_DATA.rising.second  ] ) # 'Nascer do sol'
